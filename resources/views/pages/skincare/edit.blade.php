    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="grid grid-cols-12 gap-6">
            ...
            <div class="flex flex-col col-span-full bg-white shadow-sm rounded-xl">
                ...
                <div class="p-5">
                    <form action="{{ route('skincare.update', Crypt::encryptString($skincare->id)) }}" method="POST" enctype="multipart/form-data" class="grid md:grid-cols-2 gap-5">
                        @csrf
                        @method('PATCH')
                        {{-- SINGLE GROUP INPUT --}}
                        <div class="md:col-span-2 mb-10">
                            ...
                            <div class="w-full grid grid-cols-2 gap-5">
                                {{-- LEFT --}}
                                <div class="w-full lg:h-80 rounded-md border overflow-hidden p-5">
                                    <img class="w-full h-full object-cover" src="{{ asset('storage/foto-skincare/' . $skincare->photo) }}" alt="">
                                </div>
                                {{-- RIGHT --}}
                                <div id="filename-preview" class="w-full lg:h-80 flex flex-col justify-center items-center space-x-3 bg-white border rounded-md shadow-lg p-5">
                                    ...
                                    <div class="w-full text-center py-5">
                                        ...
                                        <p class="text-sm">Anda harus menambahkan foto</p>
                                    </div>
                                </div>
                            </div>
                            @error('photo')
                            ...
                            <div class="relative flex items-center">
                                <select name="kategori_skincare" type="text" class="w-full text-gray-800 text-sm border border-gray-300 px-4 py-3 rounded-md outline-blue-600">
                                    ...
                                    @foreach ($skincareCategories as $category)
                                        <option value="{{ $category->id }}" {{ $skincare->skincare_category_id == $category->id ? 'selected' : '' }}>{{ $category->category_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div>
                            ...
                            <div class="relative flex items-center">
                                <input value="{{ $skincare->skincare_name }}" name="nama_skincare" type="text" class="w-full text-gray-800 text-sm border border-gray-300 px-4 py-3 rounded-md outline-blue-600" placeholder="Nama skincare" />
                            </div>
                            @error('nama_skincare')
                                <span class="text-xs mt-1 italic text-red-500">{{ $message }}</span>
                            @enderror
                        ...
                        <div>
                            ...
                            <div class="relative flex items-center">
                                <input value="{{ $skincare->qty }}" name="qty" type="text" class="w-full text-gray-800 text-sm border border-gray-300 px-4 py-3 rounded-md outline-blue-600" placeholder="QTY barang" />
                            </div>
                            @error('qty')
                                <span class="text-xs mt-1 italic text-red-500">{{ $message }}</span>
                            @enderror
                        ...
                        <div>
                            ...
                            <div class="relative flex items-center">
                                <input value="{{ $skincare->price }}" name="harga" type="text" class="w-full text-gray-800 text-sm border border-gray-300 px-4 py-3 rounded-md outline-blue-600" placeholder="Harga skincare" />
                            </div>
                            @error('harga')
                                <span class="text-xs mt-1 italic text-red-500">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="relative flex items-center">
                                <textarea rows="4" name="deskripsi" type="text" class="w-full text-gray-800 text-sm border border-gray-300 px-4 py-3 rounded-md outline-blue-600" placeholder="Deskripsi singkat skincare">{{ $skincare->description }}</textarea>
                            </div>
                            @error('deskripsi')
                                <span class="text-xs mt-1 italic text-red-500">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="w-full md:col-span-2 flex justify-end space-x-5">
                            <a href="{{ route('skincare.index') }}" class="flex justify-center w-32 py-3 text-sm tracking-wide rounded-lg text-white bg-slate-600 hover:bg-slate-700 focus:outline-none">Batal Edit</a>
